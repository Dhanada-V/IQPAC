...
const AUTH_TOKEN = 'stubbed_jwt_token';
...
export default function PracticeModule() {
    ...
    const fetchModuleDetails = async () => {
      try {
        ...
        if (!currentMod || currentMod.id !== moduleId) {
          ...
          currentMod = (Array.isArray(modules) ? modules : []).find((m) => m.id === moduleId);
        }
        ...
        const qRes = await fetch(`/api/practice/modules/${moduleId}/questions`, {
          headers: {
        ...
        if (!qRes.ok) {
          throw new Error(`Failed to load questions (${qRes.status}).`);
        }
        ...
        if (isMounted) {
          setErrorMessage(err.message || 'Unable to load practice questions.');
          setStatus('error');
        }
      }
    };
  ...
  const handleCheckAnswer = async () => {
    if (!selectedOption || isSubmitting || !questions[currentIndex]) return;

    setIsSubmitting(true);
    const currentQ = questions[currentIndex];

    try {
      const response = await fetch(`/api/practice/modules/${moduleId}/check-answer`, {
        method: 'POST',
        headers: {
          Authorization: `Bearer ${AUTH_TOKEN}`,
  ...
  const handleFinishPractice = async () => {
    setIsSubmitting(true);
    try {
      const response = await fetch(`/api/practice/modules/${moduleId}/submit`, {
        method: 'POST',
        headers: {
          Authorization: `Bearer ${AUTH_TOKEN}`,
          'Content-Type': 'application/json',
      ...
      alert(err.message || 'Unable to submit final practice results.');
    } finally {
      setIsSubmitting(false);
    }
  };
    ...
    <div className="page-card">
      ...
      <div className="page-header">
        {moduleInfo ? (
          <>
            ...
            <p className="page-subtitle">{moduleInfo.description || 'Practice session in progress.'}</p>
          </>
        ) : (
          <>
            <h1 className="page-title">Practice Session</h1>
            <p className="page-subtitle">Test your knowledge with practice questions.</p>
          </>
        )}
      </div>

      {status === 'loading' && (
        <p className="status-message" role="status">Loading practice questions…</p>
      )}
      ...
      {status === 'success' && questions.length === 0 && (
        <div className="status-panel" role="status">
          <p>No questions are currently available for this module.</p>
          <Link className="btn btn-secondary" to="/practice">
            Back to Practice
          </Link>
        </div>
      ...
      {status === 'success' && completionSummary && (
        <div className="completion-card">
          <h2 className="completion-title">🎉 Practice Complete!</h2>
          <p className="completion-subtitle">{completionSummary.feedback}</p>

          <div className="metrics-grid">
            <div className="metric-box">
              <span className="metric-value">{completionSummary.total_attempted}</span>
              <span className="metric-label">Questions Attempted</span>
            </div>
            <div className="metric-box metric-success">
              <span className="metric-value">{completionSummary.correct_answers}</span>
              <span className="metric-label">Correct Answers</span>
            </div>
            <div className="metric-box metric-warning">
              <span className="metric-value">{completionSummary.incorrect_answers}</span>
              <span className="metric-label">Incorrect Answers</span>
            </div>
            <div className="metric-box metric-accent">
              <span className="metric-value">{completionSummary.accuracy_percentage}%</span>
      ...
      {status === 'success' && !completionSummary && currentQuestion && (
          ...
          <div className="question-card">
            ...
            <div className="options-list" role="radiogroup" aria-label="Question Options">
              {currentQuestion.options.map((option, idx) => {
                ...
                if (feedback) {
                  if (option === feedback.correct_answer) optionClass += ' correct';
                  else if (isSelected && !feedback.is_correct) optionClass += ' incorrect';
                }
            ...
            {!feedback ? (
              ...
              <div className={`feedback-card ${feedback.is_correct ? 'feedback-correct' : 'feedback-incorrect'}`}>
